...
class BaseEnv:
    def __init__(self, render_mode="gui") -> None:
        self._configure_gl_backend(render_mode)
        self._gripper_idx = 6
    ...
    def _configure_gl_backend(self, render_mode):
        # Respect an explicit user setting if MUJOCO_GL is already defined.
    ...
    def reset(self):
        ...
        if self._render_mode == "gui":
            ...
            # Create offscreen renderer - will raise error if it fails
...
def create_ur5e_robotiq85f():
    ...
    robot = mjcf.from_path(os.path.join(script_dir, "mujoco_menagerie/universal_robots_ur5e/ur5e.xml"))
    gripper = mjcf.from_path(os.path.join(script_dir, "mujoco_menagerie/robotiq_2f85/2f85.xml"))
